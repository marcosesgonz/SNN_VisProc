...
class myImageReconstructor:
    def __init__(self, model, height, width, num_bins, device, options):
        ...
        self.DeviceTimer = CudaTimer if device.type == 'cuda' else Timer
    ...
    def initialize(self, height, width, options):
        ...
        self.event_preprocessor = EventPreprocessor(options,self.DeviceTimer)
        self.intensity_rescaler = IntensityRescaler(options,self.DeviceTimer)
    ...
    def update_reconstruction(self, event_tensor, event_tensor_id, stamp=None, save_png = False):
        with torch.no_grad():
            ...
            self.image_display(out, events) #Por defecto no mostratá las imágenes
