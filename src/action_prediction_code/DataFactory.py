...
mad_data_dir = 'dataset/MAD'
haf_data_dir = 'dataset/HAF'
...
def prepare_minibatch(data, maxlen=None, unfold_y=False):
    ...
    for seq in data:
        if type(seq[0]) is int:
            ...
            seq_new = numpy.zeros((maxlen, n_samples, seq[0].shape[1])).astype(theano.config.floatX)
            for i in range(n_samples):
                seq_new[:lengths[i], i, :] = seq[i]
...
def get_dataset(name):
    ...
    if not __datasets.has_key(name):
