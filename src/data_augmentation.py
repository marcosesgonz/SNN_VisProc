...
"""
Obtained from: https://github.com/BrainCog-X/Brain-Cog/blob/main/braincog/datasets/cut_mix.py
"""
...
def rand_bbox_time(size, rat):
    ...
    cut_t = int(step * rat)
    ct = np.random.randint(step) #Número aleatorio entre 0 y step-1
...
def rand_bbox(size, rat):
    ...
    #Pone -cut_w/2 y +cut_w/2 para los límites de la región de ancho cut_w. Si los valores se pasan de los posibles, los clipea.
...
def GMM_mask(size, rat, n=None):
    ...
    t, x, y = torch.meshgrid(t, x, y, indexing='ij') #Expandimos los arrays x,y,t en las dimensiones T,H,W. p.e: El array t tendrá todo valores 0 en t[0,:,:]

    for p in pi:
        #Se calcula el punto medio mu_n de la gaussiana: mu_n = (mt,mx,my)
        mt = np.random.randint(0, size[0])  #Nº aleatorio de todos los posbiles pasos temporales
        mx = np.random.randint(0, size[2])  #Nº aleatorio de todos los posibles valores de x
...
class CutMix(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for _ in range(self.num_mix):
            ...
            # generate mixed sample
            lam = np.random.beta(self.beta, self.beta) #Calcula la proporción de mezcla con la función beta con los dos coeficientes iguales(para que sea simétrica respecto 0.5)
            ...
            # if alpha < 0.333:
            bbx1, bby1, bbx2, bby2 = rand_bbox(img.shape, 1. - lam) #Coge una región espacial con una proporción de (1 - lam). (Clipea la región si excede los límites posibles)
            # bbx1, bby1, bbx2, bby2 = 32, 0, 48, 16

            lam = 1 - ((bbx2 - bbx1) * (bby2 - bby1) / (img.shape[-1] * img.shape[-2]))  # Proporción del área que no ha sido cogida por ranb_bbox
...
class EventMix(Dataset):
    ...
    def __init__(self,
                 dataset,           #Dataset. Dataset[index] debe contenter en la primera posición la imágen y en el segundo la etiqueta
    ...
    def create_samples(self,proportion):  #This function is created by me. In first instance, this isnt necessary.
        ...
        num_new_samples = int(len(self)*proportion)
        idx_new_samples = random.sample(idx_samples,num_new_samples)
        new_samples = []
        for i in idx_new_samples:
            new_samples.append(self[i])
